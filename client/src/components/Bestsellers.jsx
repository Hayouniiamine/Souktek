import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import API_BASE_URL from "../config";

export default function Bestsellers() {
  const [products, setProducts] = useState([]);
  const [showAllGames, setShowAllGames] = useState(false);

  useEffect(() => {
    fetch(`${API_BASE_URL}/api/products`)
      .then((res) => res.json())
      .then((data) => setProducts(data))
      .catch((err) => console.error("Failed to load products:", err));
  }, []);

  const giftCardProducts = products.filter((product) => product.type === "gift_cards");
  const gameProducts = products.filter((product) => product.type === "games");
  const displayedGames = showAllGames ? gameProducts : gameProducts.slice(0, 5);

  const getProductImageUrl = (img) => {
    if (!img) return "/images/default_image.png";
    return img.startsWith("/images") ? `${API_BASE_URL}${img}` : `${API_BASE_URL}/images/${img}`;
  };

  return (
    <section className="bg-[#0e1117] text-white py-8 px-4">
      <div className="max-w-screen-xl mx-auto">
        {/* Gift Cards Section */}
        <h2 className="text-xl font-semibold mb-4 text-white">Gift Cards and Subscriptions</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
          {giftCardProducts.map((product, index) => (
            <Link
              to={`/product/${encodeURIComponent(product.name)}`}
              key={index}
              className="bg-[#1a1d23] p-4 rounded-xl hover:shadow-lg hover:scale-105 transition duration-300 block"
            >
              <div className="aspect-square flex justify-center items-center overflow-hidden mb-3 bg-gray-800">
                <img
                  src={getProductImageUrl(product.img)}
                  alt={product.name}
                  className="object-contain w-full h-full"
                />
              </div>
              <div className="text-sm font-semibold truncate text-white">{product.name}</div>
              <div className="text-gray-400 text-xs mt-1">{product.price}</div>
            </Link>
          ))}
        </div>

        {/* Games Section */}
        <h2 className="text-xl font-semibold mt-16 mb-6 text-white">Games Keys and Accounts</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
          {displayedGames.map((product, index) => (
            <Link
              to={`/product/${encodeURIComponent(product.name)}`}
              key={index}
              className="bg-[#1a1d23] p-4 rounded-xl hover:shadow-lg hover:scale-105 transition duration-300 block"
            >
              <div className="aspect-square flex justify-center items-center overflow-hidden mb-3 bg-gray-800">
                <img
                  src={getProductImageUrl(product.img)}
                  alt={product.name}
                  className="object-contain w-full h-full"
                />
              </div>
              <div className="text-sm font-semibold truncate text-white">{product.name}</div>
              <div className="text-gray-400 text-xs mt-1">{product.price}</div>
            </Link>
          ))}
        </div>

        {/* Show More Button (Skine style) */}
        {gameProducts.length > 5 && (
          <div className="flex justify-center mt-10">
            <button
              onClick={() => setShowAllGames(!showAllGames)}
              className="px-6 py-3 bg-white text-black font-semibold rounded-full shadow-md hover:bg-gray-200 transition-all duration-300 flex items-center gap-2 group"
            >
              <span className="text-base">{showAllGames ? "Show Less" : "Show More"}</span>
              <svg
                className={`w-5 h-5 transition-transform duration-300 group-hover:translate-x-1 ${
                  showAllGames ? "rotate-180" : ""
                }`}
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                viewBox="0 0 24 24"
              >
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
          </div>
        )}

        {/* Features Section */}
        <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-8 text-center">
          <div>
            <div className="text-3xl mb-2">📧</div>
            <h3 className="font-semibold text-lg text-white"> Email delivery</h3>
            <p className="text-gray-400 text-sm mt-1">
              Your digital Gift card is ready to use within 10 minutes.
            </p>
          </div>
          <div>
            <div className="text-3xl mb-2">⭐</div>
            <h3 className="font-semibold text-lg text-white">Trusted Online Store</h3>
            <p className="text-gray-400 text-sm mt-1">
              Our store is recommended by TrustPilot users for digital purchases.
            </p>
          </div>
          <div>
            <div className="text-3xl mb-2">✅</div>
            <h3 className="font-semibold text-lg text-white">Secure & Easy</h3>
            <p className="text-gray-400 text-sm mt-1">
              Our checkout process is secure and easy to use.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
